...
class PredictivePolicy(ScalingPolicy):
    ...
    def decide(self, current_step, current_servers, current_load, predicted_load=None, cooldown_counter=0, **kwargs):
        ...
        effective_capacity = current_servers * self.capacity
        if effective_capacity == 0:
             pred_util = 1.0 if predicted_load > 0 else 0.0
        else:
             pred_util = predicted_load / effective_capacity
...
class HybridPolicy(ScalingPolicy):
    ...
    def decide(self, current_step, current_servers, current_load, predicted_load=None, cooldown_counter=0, **kwargs):
        ...
        pred_util = predicted_load / (current_servers * self.capacity) if current_servers > 0 else (1.0 if predicted_load > 0 else 0.0)
        ...
        elif pred_util < self.thresholds["in"]:
